...
class FileHandler:
    ...
    def load_high_score(self):
        global high_score, high_score_name, high_score_num
        high_score_file = open(f"{folder}high_score.txt", "a")
        high_score_file.close()
        if not self.is_empty(f"{folder}high_score.txt"):
            ...
            for i in high_score_list:
                ...
                high_score_num.append(self.deencrypt(var[1]))
    ...
    def write_high_scores(self, file1):
        global high_score_name, high_score_num
        high_score_file = open(file1, "w")
        for idx, i in enumerate(high_score_name):
            high_score_file.write(f"{i}/{self.encrypt(high_score_num[idx])}\n")
        high_score_file.close()
...
#creates an instance of the GameStateHandler class
game_state_handler = GameStateHandler()

#paths
bird_path = os.path.join(folder[:-1], "bird.png")
pipe_path = os.path.join(folder[:-1], "pipe.png")

#load high score from file
file_handler.load_high_score()
...
while True:
    ...
    while running:
        ...
        for pipe in pipe_arr:
            window1.blit(down_pipe_img, (pipe[0], pipe[1]))
            window1.blit(top_pipe_img, (pipe[0], pipe[1] - pipe_space))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    file_handler.write_high_scores(f"{folder}high_score.txt")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_BACKSPACE:
                    game_state_handler.restart_func()
